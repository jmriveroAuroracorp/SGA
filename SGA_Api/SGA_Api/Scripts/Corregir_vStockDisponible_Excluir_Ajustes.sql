-- Script para corregir vStockDisponible y excluir TempPaletLineas con TraspasoId NULL del cálculo del Reservado
...
LEFT OUTER JOIN (
    ...
    FROM (
        ...
        -- 🔷 CORREGIDO: Líneas temporales SOLO si tienen TraspasoId Y el traspaso está pendiente
        -- EXCLUIR líneas con TraspasoId NULL (ajustes de inventario que no son traspasos pendientes)
        SELECT 
            t.CodigoEmpresa,
            t.CodigoArticulo,
            t.CodigoAlmacen,
        ...
        WHERE tr.CodigoEstado IN ('PENDIENTE', 'EN_TRANSITO', 'PENDIENTE_ERP')
          ...
          -- 🔷 NUEVO: Ya no incluimos líneas con TraspasoId NULL (ajustes de inventario)
...
PRINT '   Ahora excluye TempPaletLineas con TraspasoId NULL del cálculo del Reservado.';
PRINT '   Solo cuenta como reservado las líneas asociadas a traspasos pendientes.';
GO
